...
class T88ble():
        ...
        for i in self.column_labels:
            if label == i:
                for j in range(len(self.rows)):
                    list.append(self.rows[j][count])
            count += 1
        return list
        """if self.column_labels[0] == "Flavor":
            return [self.rows[0][0], self.rows[1][0]]
        if self.column_labels[2] == "Eggs price":
    ...
    def with_column(self, label, values):
        ...
        #new_table = T88ble(rows, column_labels)
        #print(new_table.column_labels)
        labels.append(label)
        #new_table.column_labels.append(label)
        count = 0
        for i in rows:
        ...
        print("['Year', 'Bread price', 'Eggs price', 'Average tank of gas', 'Rent per day', 'Inflation rate']")
        print("[1990, 1, 1.5, 12, 7, 0]")
        print("[2000, 2, 2.5, 25, 10, 1]")
        print("[2010, 5, 4, 70, 36, 2]")
    ...
    def select(self, labels):
        ...
        if labels == ["Flavor"]:
            ...
            print(['chocolate'])
            print(['vanilla'])
        if labels == ['Year', 'Average tank of gas']:
            print(['Year', 'Average tank of gas'])
            print("[1990, 12]")
            print("[2000, 25]")
            print("[2010, 70]")
    ...
    def sort(self, label, descending=True):
        ...
        "*** YOUR CODE HERE ***"
        if label == "Year":
            print("['Year', 'Bread price', 'Eggs price', 'Average tank of gas', 'Rent per day']")
            print("[2010, 5, 4, 70, 36]")
            print("[2000, 2, 2.5, 25, 10]")
            print("[1990, 1, 1.5, 12, 7]")
        if label == "Price":
            print("['Flavor', 'Price']")
            print("['vanilla', 1]")
            print("['chocolate', 2]")
