...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    return min([c for c in centroids], key = lambda i: distance(location,i))
    # END Question 3
...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    """group by first
    how to find the minimum distance centroids group them into pairs
    first value centroid closest to
    2nd value is the restaurant itself


    restaurant values
    closest distance between the values
    group them with the distance
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        if r in reviewed:
            restaurant_name(r)
            make_dictionary[restaurant_name(r)] = user_rating(user, restaurant_name(r))
        else:
            make_dictionary[restaurant_name(r)] = predictor(r)
